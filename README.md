...
flowchart TB
  %% 主節點
  A[應用程式層<br/>(main.py)<br/><b>BookCollectionApp</b>]
  B[主界面層<br/>(main_window.py)<br/><b>MainFrame</b>]
  ...
  %% 下層三層
  C[對話框層<br/>(dialogs.py)<br/><b>BookFormDialog</b>]
  D[資料存取層<br/>(database.py)<br/><b>DatabaseManager</b>]
  E[資料模型層<br/>(models.py)<br/><b>Book</b>]
...
這個架構清楚展示了：
1. **分層結構**: 從應用程式層到資料持久層的清晰分層
2. **依賴方向**: 上層依賴下層，避免循環依賴
3. **功能模組**: 主界面的三大核心功能區塊
4. **資料流**: 從使用者輸入到資料庫存儲的完整流程
